...
void mergeList(List* l1, List* l2, List* l3){
	...
	add(l3 ,createNode(l1->begin->value));
	add(l3 ,createNode(l2->begin->value));
	n1 = l1->begin->next;
	n2 = l2->begin->next;

	int n = l1->numberOfElements + l2->numberOfElements;
	...
	for(int i = 2; i < n; i++){
		...
		printf("rodando\n");
		if(n1 != NULL && i%2 == 0){
			n3 = createNode(n1->value);
			add(l3, n3);			
			n1 = n1->next;
			printf("N1");
		}	
		if(n2 != NULL && i%2 != 0){
			n3 = createNode(n2->value);
			add(l3, n3);			
			n2 = n2->next;
			printf("N2");
		}			
